...
```{r world_bank_read}
world_bank_raw <- read_csv('../../data-raw/API_SEN_DS2_en_csv_v2_4347091.csv',skip = 3)
```
Create indicator list

```{r create_indicator_list}
# world_bank_raw %>%
#   filter(str_detect(`Indicator Name`,"expenditure|revenue"),
#          !is.na(`2017`)) %>%
#   select(`Indicator Name`,`Indicator Code`,`2017`) %>%
#   view()
...
indicators <- filter(world_bank_raw,`Indicator Code` %in% initial_indicator_set) %>%
  select(code = `Indicator Code`,
         name = `Indicator Name`)

write_csv(indicators,'../../data-raw/indicators.csv')

```

Create question list

```{r create_question_list}
name <- c(
...
code <- paste0('Q',c(1:length(name)))

questions <- tibble(code,name)
